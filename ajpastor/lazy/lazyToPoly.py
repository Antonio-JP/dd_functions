...
from sage.all import (PolynomialRing, var, factorial, Matrix)

from .conversion import ConversionSystem
from .lazyIDElements import SimpleLIDElement, ProductLIDElement, LazyIntegralDomain, SumLIDElement
        ...
        try:
            if(isinstance(element, list) or isinstance(element, set)):
                ...
                try:
                    ...
                    elif(isinstance(element, SimpleLIDElement) and len(element.get_basic_elements()) > 0):
        ...
        try:
            ...
            for key in self.__map_c_to_v.keys():
                ...
                while(c is None):
                    ...
                    if(el_init == 0 and k_init == 0):
                        i += 1
                    elif(el_init == 0 or k_init == 0):
                        break
                    else:
                        c = el_init/k_init
                if(not (c is None)):
                    d = init_value(el, 0) - c*init_value(k,0)
                    comp = (el-d)/c
                    if(comp == k):
                        ch_key = key
                        break
